...
```{r}
sx <- search_fulltext("Loxia", fq="rank:genus")
sx$data[,c("name","rank", "family")]
# View(sx$data) # provides the full set of information
```
...
```{r, eval=FALSE}
loxia_recs <- occurrences(taxon="Loxia", download_reason_id=10,
...
```{r, eval=FALSE}
loxia_recs <- ALA4R::occurrences(taxon="Loxia scotica", download_reason_id=10,
                                email="your.email@newcastle.ac.uk", verbose=TRUE)
```
...
# Only keep Loxia scotica
loxia_recs$data <- loxia_recs$data[loxia_recs$data$scientificName == "Loxia scotica",]
nrow(loxia_recs$data)
```
...
```{r}
ggplot(loxia_recs$data, aes(x=startDateYear)) +
  geom_histogram()

records_per_yr <- loxia_recs$data %>% 
...
m <- addProviderTiles(leaflet(),"Esri.WorldImagery") %>% 
  ## add markers
  addCircleMarkers(loxia_recs$data$longitudeWGS84, loxia_recs$data$latitudeWGS84,  
                   radius = 2, fillOpacity = 0.5, opacity = 0.5, col="red") %>% 
  addLegend(colors = "red", opacity=1, labels="Scottish Crossbill")
m
```
